...
(function() {
    ...
    BBoxSelector.prototype.create_label_box = function(options) {
      ...
      switch (options.input_method) {
        case 'select':
          ...
          this.label_input.append($('<option value>Choose one</option>'));
          ref = options.labels;
        ...
        case 'text':
          ...
          this.label_input = $('<input list="' + options.datalist + '" class="label_input" name="label" ' + 'type="text" value>');
          this.label_box.append(this.label_input);
  ...
  this.BBoxAnnotator = (function() {
    function BBoxAnnotator(options) {
      ...
      image_element.onload = function() {
        ...
        annotator.annotator_element.css({
          "width": options.scale * (options.width + annotator.border_width * 2) + 'px',
          "height": options.scale * (options.height + annotator.border_width * 2) + 'px',
          "cursor": "crosshair",
          "margin": "auto"
        });
        annotator.image_frame.css({
          ...
          "background-size": "100%",
          "width": options.scale * options.width + "px",
          "height": options.scale * options.height + "px",
          "position": "relative"
        });
      ...
      selector.get_input_element().blur(function(e) {
        ...
        switch (status) {
          case 'input':
            ...
            if (data.label) {
              annotator._add_entry(data);
              if (annotator.onchange) {
                annotator.onchange($.map(annotator.entries, function(entry) {
                  return {
                    "left": parseInt(entry.left / options.scale),
                    "top": parseInt(entry.top / options.scale),
                    "width": parseInt(entry.width / options.scale),
                    "height": parseInt(entry.height / options.scale),
                    "label": entry.label
                  }
                }));
              }
    ...
    BBoxAnnotator.prototype.add_entry = function(entry) {
      ...
      this.onchange($.map(this.entries, function(entry) {
        return {
          "left": parseInt(entry.left / scale),
          "top": parseInt(entry.top / scale),
          "width": parseInt(entry.width / scale),
          "height": parseInt(entry.height / scale),
          "label": entry.label
        }
      }));
      // this.onchange($.map(this.entries, function(entry) {
      //   return {
      //     "left": parseInt(entry.left / this.scale),
      //     "top": parseInt(entry.top / this.scale),
      //     "width": parseInt(entry.width / this.scale),
      //     "height": parseInt(entry.height / this.scale),
      //     "label": entry.label
      //   }));
    };

    BBoxAnnotator.prototype._add_entry = function(entry) {
      var annotator, box_element, close_button, text_box, scale = this.scale;
      ...
      close_button.click(function(e) {
        ...
        return annotator.onchange($.map(annotator.entries, function(entry) {
          return {
            "left": parseInt(entry.left / scale),
            "top": parseInt(entry.top / scale),
            "width": parseInt(entry.width / scale),
